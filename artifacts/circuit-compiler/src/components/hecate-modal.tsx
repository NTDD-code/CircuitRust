...
import {
  ...
  FlaskConical, ShieldCheck, Layers, Search,
} from "lucide-react";
...
const MODE_LABELS: Record<string, { label: string; color: string }> = {
  "vision-only":        { label: "Vision Only",        color: "#79C0FF" },
  "description-guided": { label: "Description-Guided", color: "#D29922" },
  "merged":             { label: "Vision + Guidance",  color: "#3FB950" },
};
...
export function HecateModal({ onClose, onInject, onAnalyze, loading, result, error }: HecateModalProps) {
  ...
  // Cycle through load phases while loading
  useEffect(() => {
    if (!loading) { setLoadPhase(0); return; }
    const id = setInterval(() => {
      setLoadPhase(p => (p + 1) % LOAD_PHASES.length);
    }, 1800);
  ...
  const processFile = useCallback((file: File) => {
    ...
    if (!isAccepted(file.type)) {
      setFileError("Unsupported format. Use JPEG, PNG, or WebP.");
      return;
    }
  ...
  const handleActivate = () => {
    if (b64 && mimeType) onAnalyze(b64, mimeType, description);
  };
  ...
  const confidenceColor = result
    ...
    : result.confidence >= 50 ? "#D29922" : "#F85149"
    : "#8B949E";
  ...
  const modeInfo = result ? (MODE_LABELS[result.reconstructionMode] ?? MODE_LABELS["vision-only"]) : null;
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="flex-1 overflow-auto p-5 space-y-4">
          ...
          {!result && !loading && (
            <>
              ...
              <div
                ...
                style={{ background: "#0A0D12", border: "1px solid #21262D", color: "#6E7681" }}
              >
          ...
          {loading && (
            ...
            >
                  ...
                  return (
                    <div
                      ...
                      style={{
                        ...
                        border: `1px solid ${isActive ? "#7C3AED40" : "transparent"}`,
                    ...
                    >
                      <div
                        ...
                        style={{
                          background: isActive ? "#7C3AED20" : isDone ? "#3FB95020" : "transparent",
                        }}
                      >
                        {isDone
                      ...
                      <div>
                        <p className="text-[11px] font-semibold" style={{ color: isActive ? "#E2D9F3" : isDone ? "#6E7681" : "#4A5568" }}>
                          {phase.label}
                        </p>
                        {isActive && (
                          <p className="text-[10px]" style={{ color: "#7C3AED" }}>{phase.sub}</p>
                        )}
                      </div>
                      {isActive && (
                        <Loader2 className="w-3 h-3 ml-auto animate-spin shrink-0" style={{ color: "#7C3AED" }} />
                      )}
                    </div>
                  );
                })}
              ...
              {preview && (
                <img src={preview} alt="Analyzing" className="w-24 rounded-lg opacity-30 object-contain" />
              )}
            </div>
          )}
            ...
            <div className="space-y-3">
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[11px] mt-1 leading-relaxed" style={{ color: "#8B949E" }}>
                    {result.analysis}
                  </p>
                </div>
              </div>

              {/* ── Safety Note (ugly build warning) ── */}
              {result.uglyBuildDetected && result.safetyNote && (
                <div
                  className="rounded-xl p-3.5 flex items-start gap-3"
                  style={{ background: "#1A1200", border: "1px solid #D2992260" }}
                >
                  <AlertTriangle className="w-4 h-4 shrink-0 mt-0.5" style={{ color: "#D29922" }} />
                  <div>
                    <p className="text-[11px] font-bold mb-1" style={{ color: "#D29922" }}>
                      ⚠ HECATE Physical Safety Note
                    </p>
                    <p className="text-[10px] leading-relaxed" style={{ color: "#9D8A50" }}>
                      {result.safetyNote}
                    </p>
                  </div>
                </div>
              ...
              <div className="grid grid-cols-4 gap-2">
                <div className="rounded-lg px-2.5 py-2 flex flex-col gap-0.5" style={{ background: "#0A0D12", border: "1px solid #21262D" }}>
                  <span className="text-[9px] uppercase tracking-widest" style={{ color: "#6E7681" }}>Confidence</span>
                  <span className="text-base font-bold" style={{ color: confidenceColor }}>{result.confidence}%</span>
                ...
                <div className="rounded-lg px-2.5 py-2 flex flex-col gap-0.5" style={{ background: "#0A0D12", border: "1px solid #21262D" }}>
                  ...
                  <span className="text-base font-bold" style={{ color: "#79C0FF" }}>{result.componentCount}</span>
                ...
                <div className="rounded-lg px-2.5 py-2 flex flex-col gap-0.5" style={{ background: "#0A0D12", border: "1px solid #21262D" }}>
                  <span className="text-[9px] uppercase tracking-widest" style={{ color: "#6E7681" }}>Mode</span>
                  <span className="text-[11px] font-bold leading-tight mt-0.5" style={{ color: modeInfo?.color ?? "#8B949E" }}>
                    {modeInfo?.label ?? "–"}
                  </span>
                  <span className="text-[9px]" style={{ color: "#4A5568" }}>
                    {result.uglyBuildDetected ? "ugly build" : "clean build"}
                  </span>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="rounded-xl overflow-hidden" style={{ border: "1px solid #21262D" }}>
                  ...
                  >
                    <span className="text-[10px] font-semibold" style={{ color: "#6E7681" }}>
                      BEFORE · Physical Build
                    </span>
                    {result.uglyBuildDetected && (
                      <span className="text-[9px] px-1.5 py-0.5 rounded" style={{ background: "#D2992215", color: "#D29922", border: "1px solid #D2992240" }}>
                        ugly build
                      </span>
                    )}
                  </div>
                  {preview ? (
                    <div style={{ background: "#0A0D12", position: "relative" }}>
                      <img
                        ...
                        style={{
                          maxHeight: "200px",
                          filter: result.uglyBuildDetected ? "brightness(0.85) saturate(0.7)" : "none",
                        }}
                      />
                      {result.uglyBuildDetected && (
                        <div
                          ...
                          style={{
                            background: "linear-gradient(to bottom, transparent 50%, rgba(215,153,34,0.08) 100%)",
                          }}
                        />
                      )}
                    </div>
                  ) : (
                    <div className="flex items-center justify-center h-32 text-[11px]" style={{ background: "#0A0D12", color: "#4A5568" }}>
                      No preview
                    </div>
                  )}
                </div>
                ...
                <div className="rounded-xl overflow-hidden" style={{ border: "1px solid #7C3AED30" }}>
                  <div
                    className="px-3 py-1.5 flex items-center justify-between border-b"
                    style={{ background: "#0D0A20", borderColor: "#7C3AED20" }}
                  >
                  ...
                  <pre
                    className="p-3 text-[10px] overflow-auto leading-relaxed"
                    style={{
                      background: "#080611",
                      color: "#C9D1D9",
                      maxHeight: "200px",
        ...
        {!loading && !result && (
          <div
            className="px-5 py-3.5 border-t shrink-0 flex items-center justify-between gap-3"
            style={{ borderColor: "#21262D", background: "#0A0D12" }}
          >
            <p className="text-[10px]" style={{ color: "#4A5568" }}>
              Powered by Gemini Vision · No data stored
            </p>
